package com.jlwteam.rebate.base.aframe;
 ...
 * @author yhb
 */
public abstract class CpHttpFrameXrvFragment<T> extends CpHttpFrameRvFragment<T> implements ExRvItemViewHolderFooter.ILoadMoreListener,
    ...
    protected void initRecycleView(ExRecyclerView erv) {
...
//            ((SimpleItemAnimator) erv.getItemAnimator()).setSupportsChangeAnimations(false);
    ...
    protected void initSwipeView(SqkbSwipeRefreshLayout swipeView) {
    ...
    protected void setLoadMoreFailedTipText(String tipText){
    ...
    protected void setLoadMoreUseInsertNotify(boolean useInsert){

        mLoadMoreUseInsertNotify = useInsert;
    }

    protected void setLoadMoreForever(boolean forever){

        mLoadMoreForever = forever;
    }
     ...
     * @author yhb
     */
    public final class PullRefreshHttpTaskListener extends JzydJsonListener<T> {
...
//                return super.onTaskSaveCache(resp);
    ...
    protected void invalidateContentRefreshRecyclerView(List<?> result, boolean isAddAll) {
        ...
        if (isAddAll) {
            ...
            boolean hasLoadMore = mLoadMoreForever ? true : onRefreshRecyclerViewCheckLoadMoreEnable(result);
            getRecyclerView().setLoadMoreEnable(hasLoadMore);
            if(mLoadMoreUseInsertNotify)
                adapter.notifyItemRangeInserted(rangeStart, CollectionUtil.size(result));
            else
